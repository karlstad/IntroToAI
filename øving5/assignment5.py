...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        """
        global backtrack_called
        global backtrack_failed
        backtrack_called += 1
        finished = True
        for variable, domain in assignment.iteritems():
			if (len(domain) != 1):
				finished = False
				break
        ...
        for value in assignment[var]:
            domain = copy.deepcopy(assignment)
            domain[var] = [value]
            if(self.inference(domain, self.get_all_arcs())):
                result = self.backtrack(domain)
                if(result):
                    return result
    ...
    def inference(self, assignment, queue):
        ...
        while(queue):
            ...
            if(self.revise(assignment, x_i, x_j)):
                ...
                all_neighbors.remove((x_j,x_i))
                for (x_k, _) in all_neighbors:
    ...
    def revise(self, assignment, i, j):
        ...
        for x_i in assignment[i]:
            ...
            for x_j in assignment[j]:
                if((x_i, x_j) in self.constraints[i][j]):
                    satisfied = True
            if(not satisfied):
...
print "Number of backtrack calls ", backtrack_called
print "Number of backtrack fails ", backtrack_failed